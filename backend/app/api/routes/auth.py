...
@router.post(
    ...
    response_model=TokenResponse,
    summary="Sign in",
    description="Validates credentials, sets an HttpOnly refresh cookie, and returns a short-lived access token.",
    responses=_error,
)
def login(
...
@router.post(
    ...
    description="Updates the password after verifying the current one. Other sessions are revoked.",
    responses={401: {"model": ErrorResponse}, 422: {"model": ErrorResponse}},
)
def change_password(
...
@router.post(
    ...
    response_model=MessageResponse,
    summary="Sign out",
    description="Revokes the refresh session, clears the refresh cookie, and invalidates access tokens.",
)
def logout(
    request: Request,
    response: Response,
